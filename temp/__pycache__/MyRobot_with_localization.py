...
from .module.MyPlanning import PIDVec, point_to_path_min_distance_nd, smooth_path_nd
from .module.MyControl import MotionModel, SimpleBicycleModel
from .module.MyUtils import _vec, _check_dim, _wrap_pi
from .module.MySlam import SlamMap
from .module.MyNavi import astar_find_path_center_nodes, euclidean
...
class Moblie_robot:
    ...
    def __init__(
        ...
        self._use_estimated_pose: bool = True
        # estimated orientation (heading) used for heading-term control
        self._ori_est: Tuple[float, ...] = tuple(0.0 for _ in range(dim))
        self._pos_est_prev: Tuple[float, ...] = self._pos_est
    ...
    def get_estimated_orientation(self) -> Tuple[float, ...]:
        # \"\"\"센서 기반 추정 orientation(heading).\"\"\"
    ...
    def _update_estimated_pose(self) -> None:
        ...
        if not self._sensing_mode or self._sensor is None:
            ...
            self._pos_est_prev = self._pos_est
        ...
        # update estimated orientation using delta of estimated positions
        px, py = self._pos_est_prev[0], self._pos_est_prev[1]
        nx, ny = self._pos_est[0], self._pos_est[1]
        dx, dy = (nx - px), (ny - py)
        if abs(dx) + abs(dy) > 1e-9:
            th = math.atan2(dy, dx)
            self._ori_est = (float(th), 0.0)
        else:
            # if no movement in estimate, keep previous heading
            self._ori_est = self._ori_est
        self._pos_est_prev = self._pos_est
    ...
    def _step(self, dt_ns: int, note: str = "") -> None:
        ...
        # keep estimated orientation sane when not using sensing-based updates
        if (not self._sensing_mode) or getattr(self, "_test_map_mode", False):
            self._ori_est = self._orientation
            self._pos_est_prev = self._position

        # (localization) periodically update estimated pose
        if self._sensing_mode and (self._tick_count % self._loc_period == 0):
